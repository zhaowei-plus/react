...
export function requestUpdateLane(fiber: Fiber): Lane {
    ...
    } else if (!deferRenderPhaseUpdateToNextBatch &&
...
export function scheduleUpdateOnFiber(
    ...
    if (root === workInProgressRoot) {
        ...
        if (workInProgressRootExitStatus === RootSuspendedWithDelay) {
            ...
            // 根目录已经延迟了暂停，这意味着此渲染 肯定不会完成。 由于我们有新的更新，因此将其标记为
            // 在标记即将来临的更新之前立即暂停。 这有 中断当前渲染并切换到更新的效果。

            // 待办事项：请确保此操作不会覆盖我们执行操作时发生的ping操作已经开始渲染。

            // TODO markRootSuspended
...
function markRootSuspended(root, suspendedLanes) {
    ...
    // 挂起时，我们应始终排除在渲染阶段进行的或（较谨慎地，因为我们尽量避免）更新的通道。

    // TODO: Lol maybe there's a better way to factor this besides this obnoxiously named function :)
    // TODO：大声笑也许除了这个令人讨厌的函数之外，还有更好的方法来分解它：)
...
function commitRootImpl(root, renderPriorityLevel) {
    ...
    // a scheduled timeout.
    if (rootsWithPendingDiscreteUpdates !== null) {
        if (!hasDiscreteLanes(remainingLanes) &&
